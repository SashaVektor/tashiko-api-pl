...
export async function getAccessToken() {
  ...
  if (!process.env.TPAY_CLIENT_ID || !process.env.TPAY_CLIENT_SECRET) {
    throw new Error('Tpay credentials are not configured')
  }
...
export async function verifyTpayJwsSignature(req, rawBodyBuffer) {
  ...
  let certificateUrl
  try {
    certificateUrl = new URL(x5u)
  } catch {
    throw new Error('Invalid x5u')
  }
  if (
    certificateUrl.protocol !== 'https:' ||
    certificateUrl.hostname !== 'secure.tpay.com'
  ) {
    throw new Error('Invalid x5u')
  }

  // Скачиваем сертификат из x5u и верифицируем подпись содержимого
  const certResp = await axios.get(certificateUrl.toString(), {
